...
###############################################################################
    def load_results(self, table):
        ...
        for list in table[1:]:
            data_dict = {}
            data_dict[heading_1] = float(list[1])
            data_dict[heading_2] = float(list[2])
            data_dict[heading_3] = float(list[3])
            d[list[0]] = data_dict
...
###############################################################################
    ...
    def horse_fastest_race(self, horse):
        ...
        # check if horse doesn't exist
        if horse not in self.race_dict:
            return (race_name, fastest_time)
